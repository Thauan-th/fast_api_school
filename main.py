from Resources.Student import student_resources
...
def all_users():
    try:
        return student_resources["index"]()
    except Exception:
        raise HTTPException(
...
def find_user(id: int = Path(default=None, title="Student ID", description="ID to find a studant")):
    try:
        return student_resources["show"](id)
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Bad Request")
...
def new_user(student: Student = Body(default=None, title="Studant Informations", description="Studant registration information")):
    try:
        return student_resources["create"](student)
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Bad Request")
...
def update_user(id: int = Path(default='None', title="Student ID", description="ID to find the user to be updated"),
    ...
    try:
        return student_resources["update"](id, student)
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Bad Request")
...
def delete_user(id: int = Path(default=None, title="Student ID", description="Student to be deleted")):
    try:
        return student_resources["delete"](id)
    except Exception:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Bad Request")
